            ...
            <b-table-column label="Actions">
              <b-dropdown aria-role="list">
                ...
                <b-dropdown-item has-link aria-role="listitem">
                  ...
                  <router-link :to="`transactions/credit/User/${props.row.id}/Deposists`">deposits</router-link>
                  <router-link
                    :to="`transactions/debit/User/${props.row.id}/Withdrawals`"
                  ...
                  <router-link :to="`activity/event/Event/${props.row.id}`">My Events</router-link>
                  <router-link :to="`activity/meeting/Meeting/${props.row.id}`">My Meetings</router-link>
                  <router-link :to="`project/${props.row.id}`">My Projects</router-link>
                </b-dropdown-item>
              </b-dropdown>
            </b-table-column>
          </template>
    ...
    members() {
      if (this.group_id && this.type === "admins")
        return this.$store.getters["GroupInfo/members"].filter((m) => {
          return m.group_id === parseInt(this.group_id) && m.is_admin === 1;
        });
      ...
      if (this.group_id && this.type === "loan-approvers")
        return this.$store.getters["GroupInfo/members"].filter((m) => {
          return (
            m.group_id === parseInt(this.group_id) && m.loan_approver === 1
          );
        });
