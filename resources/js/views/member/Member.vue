<template>
    <div>
        <hero-bar :has-right-visible="true">Members</hero-bar>
                    ...
                    <template slot-scope="props">
                        ...
                        >
                            <span
                                class="tag"
                                :class="
                                    props.row.active ? 'is-success' : 'is-grey'
                                "
                                >{{
                        ...
                        <b-table-column label="Actions">
                            <b-dropdown aria-role="list">
                                ...
                                <b-dropdown-item has-link aria-role="listitem">
                                    <router-link
                                        :to="`member/card/${props.row.id}`"
                                        >View details</router-link
                                    >
                                    <router-link
                                        :to="`transactions/credit/User/${props.row.id}`"
                                        >deposits</router-link
                                    >
                                    <router-link
                                        :to="`transactions/debit/User/${props.row.id}`"
                                        >withdrawals</router-link
                                    >
                                    <router-link
                                        :to="`pending-payments/${props.row.id}/user`"
                                        >Pending payment</router-link
                                    >
                                    <router-link :to="`/groups/${props.row.id}`"
                                        >My Groups</router-link
                                    >
                                    <router-link
                                        :to="`/wallets/User/${props.row.id}`"
                                        >My Wallet</router-link
                                    >
                                    <router-link :to="`/nok/${props.row.id}`"
                                        >Next of Kin</router-link
                                    >
                                    <router-link
                                        :to="`/loans/all/user/${props.row.id}`"
                                        >My Loans</router-link
                                    >
                                    <router-link
                                        :to="`activity/event/Event/${props.row.id}`"
                                        >My Events</router-link
                                    >
                                    <router-link
                                        :to="`activity/meeting/Meeting/${props.row.id}`"
                                        >My Meetings</router-link
                                    >
            ...
            <form action>
                <div class="modal-card" style="width: auto;">
                    ...
                    <footer class="modal-card-foot">
                        <a class="button" @click="isModalActivateActive = false"
                            >Close</a
                        >
                        <a
                            class="button is-primary"
                            @click.prevent="onOkToggleActive"
                            >Save</a
                        >
                    </footer>
                </div>
...
export default {
    ...
    beforeRouteEnter(to, from, next) {
        next((v) => {
            if (to.params.id) {
                v.$store.dispatch("GroupInfo/getMembers", to.params.id);
            } else {
    ...
    computed: {
        members() {
            if (this.group_id && this.type === "admins")
                return this.$store.getters["GroupInfo/members"].filter((m) => {
                    return (
                        m.group_id === parseInt(this.group_id) &&
                        m.is_admin === 1
                    );
                });
